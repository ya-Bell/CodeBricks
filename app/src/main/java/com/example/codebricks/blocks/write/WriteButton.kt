...
@Composable
fun CreateWriteBlock(viewModel: VariableViewModel) {
    ...
    if (showDialog.value && viewModel.variables.isNotEmpty()) {
        AlertDialog(
            ...
            title = { Text(text = stringResource(id = R.string.select_variable_to_write)) },
            text = {
                Column {
                    viewModel.variables.forEach { variable ->
                        Button(
                            onClick = {
                                viewModel.declareWriteBlock(variable)
